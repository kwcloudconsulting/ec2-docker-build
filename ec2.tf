...
resource "null_resource" "name" {
  ...
  connection {
    type        = "ssh"
    user        = "ec2-user"
    private_key = file("~/Users/karimarie/AWSkeypair.pem")
  ...
  provisioner "file" {
    source      = "/Users/karimarie/Downloads/my_password.txt"
  ...
  provisioner "file" {
    ...
    destination = "/home/ec2-user/builddockerimage.sh"
  }

  # set permissions and run the build_docker_image.sh file
  provisioner "remote-exec" {
    inline = [
      "sudo chmod +x /home/ec2-user/builddockerimage.sh",
      "sh /home/ec2-user/builddockerimage.sh",
    ]
  }
